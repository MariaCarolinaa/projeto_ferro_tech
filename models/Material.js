...
class Material {
    ...
    try {
      const materials = knex
        .select([
          ...
          "m.estoque",
          "tm.tipo",
          "tm.unidade_referencia",
          "tm.preco",
        ])
        .from({ m: "material" })
        .where({usuario_material})
  ...
  async findTypeMaterials() {
    try {
      const materials = knex
        .select(["id_tipo", "tipo", "preco", "unidade_referencia"])
        .table("tipo_material")
        .groupBy("tipo");
      return materials;
    } catch (err) {
  ...
  async create(material) {
    try {
      ...
      const result = await knex.transaction(async (trx) => {
        const id = await knex
          .insert(tipo_material)
          .table("tipo_material")
          .transacting(trx);
        await knex
          .insert({ ...dados, tipo: id })
          .table("material")
          .transacting(trx);
      });
      return result;
